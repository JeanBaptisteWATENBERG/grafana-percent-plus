...
function SpanValue({fontSizePercent, className, children}: PropsWithChildren<{fontSizePercent: string, className: string}>) {
  ...
  return <span className={className} style={{fontSize: pixelSize + 'px'}}>{children}</span>
}
...
export class PercentPanel extends PureComponent<Props> {
  render() {
    ...
    return <div className="singlestat-panel" style={{background: getColorFromHexRgbOrName(applyableThreshold.color)}}>
      <div className="singlestat-panel-value-container">
        <SpanValue className="singlestat-panel-value" fontSizePercent={options.valueFontSize}>{options.decimal !== -1 ? percent.toFixed(options.decimal) : percent}</SpanValue>
        <SpanValue className="singlestat-panel-postfix" fontSizePercent={options.percentFontSize}>%</SpanValue>
      </div>
    </div>;
  }
}
